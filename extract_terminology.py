...
# 「KRAFTON Japan 表記ルール」ページID(ユーザーが手動作成・インテグレーション共有済み)
# find_style_rules_page失敗時のフォールバックとして直接指定するページID。
...
_PID_FILE = _SCRIPT_DIR / "extract_terminology.pid"
_CHECKPOINT_FILE = _SCRIPT_DIR / "extract_terminology_checkpoint.json"
...
def _save_checkpoint(candidates: list[dict], last_batch: int) -> None:
    tmp = str(_CHECKPOINT_FILE) + ".tmp"
    with open(tmp, "w", encoding="utf-8") as f:
        json.dump({"last_batch": last_batch, "candidates": candidates}, f, ensure_ascii=False)
    Path(tmp).replace(_CHECKPOINT_FILE)
    print(f"  💾 チェックポイント保存（バッチ{last_batch}、候補{len(candidates)}件）", flush=True)
...
def _load_checkpoint() -> tuple[list[dict], int]:
    if _CHECKPOINT_FILE.exists():
        with open(_CHECKPOINT_FILE, encoding="utf-8") as f:
            data = json.load(f)
        print(f"⏩ チェックポイント復元: バッチ{data['last_batch']}まで、候補{len(data['candidates'])}件")
        return data["candidates"], data["last_batch"]
    return [], 0
...
def extract_terms_and_style(text_ja: str, client: Anthropic,
                             text_kr: str = "", text_en: str = "") -> dict:
...
def consolidate_style_rules(candidates: list[dict], client: Anthropic) -> list[dict]:
    ...
    consolidated = parse_consolidation_response(raw_text)
...
def get_or_create_style_rules_page(notion: Client) -> tuple[str, str]:
    ...
    Notion search APIはインデックスラグやIntegration権限の問題でページを見つけられない場合がある。
    search失敗時はNOTION_STYLE_RULES_PAGE_IDで直接アクセスしてフォールバックする。
    既存ページに「要確認」見出しがまだない場合は自動追加する。
    """
    page_id = find_style_rules_page(notion)
    if not page_id and NOTION_STYLE_RULES_PAGE_ID:
        try:
            notion.pages.retrieve(page_id=NOTION_STYLE_RULES_PAGE_ID)
            page_id = NOTION_STYLE_RULES_PAGE_ID
        except Exception:
            pass
    if page_id:
...
def process_batch(batch: list[dict], known_ja: set[str], style_candidates: list[dict],
                   notion: Client, client: Anthropic, dry_run: bool) -> bool:
    """1バッチ（本文取得済みrelease群）を処理する。
    Claude API呼び出しが2回失敗した場合はFalseを返し、呼び出し側は「用語抽出済み」をONにしない。
    """
    combined_text = "\n\n---\n\n".join(r["body"][:3000] for r in batch)

    result = None
    for attempt in range(2):
        try:
            result = extract_terms_and_style(combined_text, client)
            break
...
def main():
    ...
    style_candidates, last_completed_batch = _load_checkpoint()
    current_batch_idx = last_completed_batch
    processed_count = 0
    ...
    try:
        ...
        for i, batch in enumerate(batches, 1):
            ...
            if i <= last_completed_batch:
                print(f"[バッチ {i}/{len(batches)}] チェックポイント済みのためスキップ")
                continue
            print(f"\n[バッチ {i}/{len(batches)}] {len(batch)}件", flush=True)
            try:
                ready = fetch_bodies_for_batch(batch)
                if not ready:
                    print("  ⚠️  本文を取得できたreleaseがないためスキップ")
                    continue
                if process_batch(ready, known_ja, style_candidates, notion, anthropic_client, args.dry_run):
            ...
            if i % _CHECKPOINT_INTERVAL == 0:
                _save_checkpoint(style_candidates, i)
        ...
        try:
            consolidated = consolidate_style_rules(style_candidates, anthropic_client)
        ...
        _save_checkpoint(style_candidates, current_batch_idx)
        print(f"\n⚠️  中断。チェックポイントを保存しました（バッチ{current_batch_idx}まで）")
        sys.exit(1)
    finally:
